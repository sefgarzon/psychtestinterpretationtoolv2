...
// Smart picker: checks if there are multiple options (array) or just one (string)
function pickText(dataElement) {
    if (Array.isArray(dataElement)) {
        return shuffle(dataElement); // If it's an array, pick a random one
    }
    return dataElement; // If it's already a single string, return it as-is
}
...
function interpretEQ() {
    ...
    if (!box) return; // Guard clause against missing DOM element
    
    const gender = document.getElementById('gender').value;
    
    // 1. Get the dropdown values for all 4 sub-traits
    const intraVal = document.getElementById('eq_intra')?.value || "";
    const interVal = document.getElementById('eq_inter')?.value || "";
    const stressVal = document.getElementById('eq_stres')?.value || "";
    const adaptVal = document.getElementById('eq_adapt')?.value || "";

    // Priority 1: Check if ANY dropdown is set to "invalid"
    const isInvalid = (intraVal === 'invalid' || interVal === 'invalid' || stressVal === 'invalid' || adaptVal === 'invalid');
    
    if (isInvalid) {
        // Pulls the standard invalid paragraph from data.js
        const invalidText = WPC_DATA.eq.intrapersonal.invalid[gender];
        box.innerText = format(invalidText);
        return; 
    }

    // Priority 2: If all selectors are empty, show the placeholder dots
    if (!intraVal && !interVal && !stressVal && !adaptVal) {
        box.innerText = "...";
        return;
    }
    ...
    // 2. Extract narrative text for Intrapersonal
    if (intraVal) {
        const raw = WPC_DATA.eq.intrapersonal[intraVal][gender];
        if (raw) paragraphs.push(pickText(raw));
    }

    // 3. Extract narrative text for Interpersonal
    if (interVal) {
        const raw = WPC_DATA.eq.interpersonal[interVal][gender];
        if (raw) paragraphs.push(pickText(raw));
    }

    // 4. Extract narrative text for Stress Management
    if (stressVal) {
        const raw = WPC_DATA.eq.stressManagement[stressVal][gender];
        if (raw) paragraphs.push(pickText(raw));
    }

    // 5. Extract narrative text for Adaptability
    if (adaptVal) {
        const raw = WPC_DATA.eq.adaptability[adaptVal][gender];
        if (raw) paragraphs.push(pickText(raw));
    }

    // Priority 3: Combine everything into clean paragraphs
    let formattedParagraphs = paragraphs.map((text) => {
        let trimmed = text.trim();
        return trimmed.charAt(0).toUpperCase() + trimmed.slice(1);
    });

    // Join with double line breaks for distinct visual blocks inside your text area
    let finalNarrative = formattedParagraphs.join(" ");
    box.innerText = format(finalNarrative);
}
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Setup BPI Traits
    const traits = ["Hyp", "Dep", "Axy", "IPs", "Aln", "ImE", "PId", "ThD", "SoI", "SDp", "Dev", "Den"];
